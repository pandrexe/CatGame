...
public class TaskManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (InteractableTask task in tuttiI_Task)
        {
            ...
            {
                numeroTotaleTask++; // Lo aggiungiamo al totale della run solo se � valido!
            ...
            {
                Debug.Log($"[TaskManager] Ignorato l'oggetto '{task.gameObject.name}' perch� impostato su TaskType.Nessuno");
            }
        }
    ...
    // Questa funzione verr� usata dal tuo BedInteraction per capire se la run � valida
